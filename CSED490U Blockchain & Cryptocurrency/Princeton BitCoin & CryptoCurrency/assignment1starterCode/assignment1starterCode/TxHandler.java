...
public class TxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
    	...
    	for(int i=0; i < tx.numInputs(); i++) {
    		...
    		//we are using utxo to get the transaction output;
    		UTXO utxo = new UTXO(in.prevTxHash, in.outputIndex);
    		Transaction.Output output = utxoPool.getTxOutput(utxo);
    		
    		//since the transaction output is being received from utxo object,
    		//we can verify transaction output using the utxo object
    		if(!utxoPool.contains(utxo))
    			return false;
    		
    		//to verify all the signatures in the input are valid
    		if(! Crypto.verifySignature(output.address, tx.getRawDataToSign(i), in.signature))
    			return false;
    		
    		//to verify that the UTXO are not been repeated.
    	...
    	for(Transaction.Output out: tx.getOutputs()) {
    		//to verify that all of the transaction outputs are non-negative
    		if(out.value < 0)
    			return false;
    		valueProducedSum += out.value;
    	}
    	//to verify that the sume of transaction input values is greater than or equal to the output values 
