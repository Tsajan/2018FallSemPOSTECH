...
#array to insert data from training set
Time=[]
Hour=[]
Minute=[]
Month=[]
...
#array to insert data from test set
Time1=[]
Hour1=[]
Minute1=[]
Month1=[]
...
df_train=pd.DataFrame.from_csv("crime_train.csv",index_col=None,parse_dates=True)
df_test=pd.DataFrame.from_csv("crime_test.csv",index_col=None,parse_dates=True)
...
def DecisionTreeClassifier(TrainData,TestData):
    ...
    cats=pd.DataFrame(predicted, columns=le_crime.classes_)
    id_col = df_test[['id']].copy()
    id_col = id_col.astype(float).astype(str)
    result = pd.concat([id_col, cats], axis=1)
    result.to_csv('sample_crime_submission_20182095.csv',index=False)
